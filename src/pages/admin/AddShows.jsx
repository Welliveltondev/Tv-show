...
import {CheckCheckIcon, DeleteIcon, StarIcon} from 'lucide-react'
...
const AddShows = () => {
  ...
  return nowPlayingMovies.length > 0 ? (
    <>
            ...
            onClick={()=>setSelectedMovie(movie.id)}>
              ...
              {selectedMovie === movie.id && (
                <div className='absolute top-2 right-2 flex items-center justify-center bg-primary h-6 w-6 rounded'>
                  <CheckCheckIcon className='w-4 h-4 text-white' strokeWidth={2.5}/>
                </div>
              )}
      ...
      <button className='bg-primary text-white px-8 py-2 mt-6 rounded hover:bg-primary-dull/90 transition-all cursor-pointer'>
        Add Show
      </button>
    </>
  ) : (
