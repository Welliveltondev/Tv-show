    ...
    <div className='h-[calc(100vh-64px)] md:flex flex-col items-center pt-8 max-w-13 md:max-w-60 w-full border-r border-gray-300/20 text-sm'>
      ...
      <div className='w-full'>
        {adminNavLinks.map((link, index)=>(
          <NavLink className={({isActive}) => `relative flex items-center max-md:justify-center gap-2 w-full py-2.5 min-md:pl-10 first:mt-6 text-gray-400 ${isActive && 'bg-primary/15 text-primary group'}`}
          end  key={index} to={link.path}>
            {({isActive}) =>(
              <>
