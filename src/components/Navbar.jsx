...
const Navbar = () => {
  ...
  const {user} = useUser()
  const {openSiginIn} = useClerk()
  const navigate = useNavigate()

  return (
    <div className='fixed top-0 left-0 z-50 flex items-center justify-between px-6 md:px-16 lg:px-36 py-5 w-full'>
        ...
        !user ? (
          ...
          <UserButton>
            <UserButton.MenuItems>
              <UserButton.Action label='My Bookings' labelIcon={<TicketPlus with={15} />} onClick={()=> navigate('/my-bookings')}/>
            </UserButton.MenuItems>
          </UserButton>
        )
      }
